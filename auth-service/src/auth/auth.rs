...
use crate::domain::{email::Email, constants::JWT_COOKIE_NAME};
...
mod tests {
    ...
    async fn test_generate_auth_cookie() {
        let email = Email::parse("test@example.com".as_ref()).unwrap();
    ...
    async fn test_create_auth_cookie() {
        ...
        let cookie = crate::auth::auth::create_auth_cookie(token.clone());
        assert_eq!(cookie.name(), JWT_COOKIE_NAME);
    ...
    async fn test_generate_auth_token() {
        let email = Email::parse("test@example.com".as_ref()).unwrap();
        let result = crate::auth::auth::generate_auth_token(&email).unwrap();
    ...
    async fn test_validate_token_with_valid_token() {
        let email = Email::parse("test@example.com".as_ref()).unwrap();
        let token = crate::auth::auth::generate_auth_token(&email).unwrap();
